...
def write(excelPath, basePath, adingList):

    # list -> pandas
    df_adingList = pd.DataFrame(adingList, columns=['answer'])
    ...
    df_adingList.to_excel(writer, startcol=3, index=False)
    writer.save()
    return 0
...
for i_data in data_lists:
    ...
    # adding data
    adingList = np.zeros(imgList.shape, dtype=np.int32)

    write(i_data, basePath=basePath, adingList=adingList)
